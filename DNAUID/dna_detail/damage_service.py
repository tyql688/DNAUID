...
async def resolve_damage_build(
    dna_user: DNAUser,
    build: RoleDamageBuild,
) -> tuple[RoleDamageBuild, str | None]:
    """解析用于伤害计算的角色技能，返回过滤后的 build 与错误提示。

    角色详情可能返回 4 个技能（含 lv1 大招/被动），但服务端伤害计算只接受
    /role/build/config 中该角色 skillIds 声明的主技能（通常 3 个）。
    直接发送全部技能会触发 10000 参数错误（如止流、艾达等多技能角色），
    渲染阶段复用同样的 3 技能校验也会抛 ValueError。

    这里按 buildConfig 过滤 role_detail.skills，返回只含声明技能的 build：
    - config 获取失败：降级返回原 build（保留旧行为，避免 config 故障全员失效）
    - 该角色 skillIds 为空：返回明确错误（服务端不支持该角色伤害计算）
    - 否则：返回 role_detail.skills 已过滤为 3 个声明技能的 build

    返回 (用于计算与渲染的 build, 错误提示或 None)。
    """
    role_detail = build.role_detail
    ...
    if not skill_ids_str:
        return build, f"角色 {role_detail.charName} 暂不支持伤害计算"
    ...
    if len(filtered) != 3:
        return build, f"角色 {role_detail.charName} 的伤害计算技能配置异常"
    filtered_detail = role_detail.model_copy(update={"skills": filtered})
    return dataclasses.replace(build, role_detail=filtered_detail), None
...
async def calculate_role_damage(
    ...
    build, filter_err = await resolve_damage_build(dna_user, build)
