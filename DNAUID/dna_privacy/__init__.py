...
@sv_dna_privacy.on_fullmatch("显示UID")
async def disable_uid_hidden(bot: Bot, ev: Event):
    """个人关闭UID隐藏"""
    # 检查组是否有强制设置
    if ev.group_id:
        error_msg = await _check_group_force_uid_hidden(ev.group_id, ev.bot_id)
...
async def enable_uid_hidden_admin(bot: Bot, ev: Event):
    """群管理员开启被@玩家的UID隐藏"""
    if not ev.group_id:
        return await bot.send("请在群聊中使用此命令")
...
async def disable_uid_hidden_admin(bot: Bot, ev: Event):
    """群管理员关闭被@玩家的UID隐藏"""
    if not ev.group_id:
        return await bot.send("请在群聊中使用此命令")
...
async def enable_uid_hidden_all(bot: Bot, ev: Event):
...
async def disable_uid_hidden_all(bot: Bot, ev: Event):
...
async def cancel_uid_hidden_all(bot: Bot, ev: Event):
